...
def make_gf(mesh):
    iw_vec = array([iw.value * np.eye(n_idx) for iw in mesh])
    G0 = BlockGf(mesh=mesh, gf_struct=gf_struct)
    G0_k = linalg.inv(iw_vec[None,...] + mu_mat[None,None,...] - e_k_vec.data[::,None,...])
    G0['bl'].data[:] = np.sum(G0_k, axis=0) / len(k_mesh)
    Delta = G0.copy()
    Delta['bl'] << iOmega_n + mu_mat - h_0_mat - inverse(G0['bl'])
    return G0, Delta
