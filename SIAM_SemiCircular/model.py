...
from triqs.gf import Gf, MeshImFreq, MeshReFreq, MeshReFreqPts, MeshReFreqLog, Omega, iOmega_n, inverse, SemiCircular
...
def make_g0_and_delta(mesh):
    ...
    G0['up'] << inverse(z + mu + h - SemiCircular(D))
    G0['dn'] << inverse(z + mu - h - SemiCircular(D))
    Delta = G0.copy()
    Delta['up'] << z + mu + h - inverse(G0['up'])
    Delta['dn'] << z + mu - h - inverse(G0['dn'])
    return G0, Delta
