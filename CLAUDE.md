...
- **Docker usage**: Build with `docker build -t solver_benchmarks .` (use `--build-arg NCORES=16` for faster builds). Run with `docker run --rm -u 0:0 -v $(pwd):/home/triqs/benchmarks solver_benchmarks <cmd>`. The `-u 0:0` is needed due to Docker user namespace remapping causing UID mismatch with bind mounts. Results are written to `MODEL/results/` on the host via the bind mount. For interactive use: `docker run --rm -it -u 0:0 -v $(pwd):/home/triqs/benchmarks solver_benchmarks`.
